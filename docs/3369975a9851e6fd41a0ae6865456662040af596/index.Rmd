...
An operating model is a theoretical description of fishery and population dynamics used for the testing of management strategies that could include, for example,  data collection protocols, stock assessment methods, harvest control rules, enforcement policies and reference points. Typically, closed-loop simulation refers to the use of operating models for testing management procedures (aka. harvest strategy) that are any codifable rule for setting management advice from data. Management Strategy Evaluation is a wider process of stakeholder and manager engagement that identifies system uncertainties, performance metrics, viable management procedures, aiming to adopt an MP for the provision of management advice for an established time period. 

#### Reference Case Operating Models

This document describes the specification of a reference case operating model including links and references to literature. The reference case operating model is used as the single 'base' operating model from which reference set and robustness set operating models are specified. Reference and robustness tests are typically 1-factor departures from the reference case OM, however sometimes reference set OMs are organized in a factorial grid across primary axes of uncertainty. 
...
The role of the reference set operating models is to provide the central basis for evaluating harvest strategy performance, for example rejecting badly performing harvest strategies. 

#### Robustness Set Operating Models

Robustness set operating models are intended to include additional sources of uncertainty for providing further discrimination among harvest strategies that perform comparably among reference set operating models. 
...
Table 1a. Project Updates and Progress
```{r}
getprog("Geoduck")
```
...
Table 1b. Geoduck RCM model fits 
```{r}
...
Sea cucumber operating models were configured to be numbers-based due to the inability to age sea cucumbers and representatively measure/weigh them. This entails the following working assumptions: knife-edge growth to size / weight 1 at age 2; Recruitment based on a Beverton-Holt S-R relationship calculated from mature numbers (although this can be modified later); Asymptotic selectivity from a young age class (e.g. age 4-7).
...
Models were conditioned using RCM and fitted to historical catches, an estimate of absolute biomass, and a time series of survey density estimates.  

The Reference Case operating model presented here is for Quota Management Area 25A (Nootka / Tahsis on the WCVI, Subareas 25-6, 25-8, 25-9, 25-12, 25-15) which was selected for its relatively long history of density surveys and recent reopening in 2020.
